...
class TestDailyOutputRecord(FrappeTestCase):
    ...
    def test_enqueue_throws_if_in_progress(self):
        """enqueue_process_all throws if processing_status is In Progress"""
        self.dor.processing_status = "In Progress"
        self.dor.save()

        with self.assertRaises(frappe.ValidationError):
            self.dor.enqueue_process_all()

    @patch("frappe.get_doc")
    @patch("caf.caf.doctype.daily_output_record.daily_output_record.frappe.enqueue")
    def test_enqueue_sets_status_and_queues(self, mock_enqueue, mock_get_doc):
        """enqueue_process_all sets status to In Progress and calls frappe.enqueue"""
        mock_get_doc.return_value = self.dor

        result = self.dor.enqueue_process_all()

        self.assertTrue(result["queued"])
        self.assertEqual(
            frappe.db.get_value("Daily Output Record", self.dor.name, "processing_status"),
            "In Progress",
        )
        mock_enqueue.assert_called_once()

    @patch("caf.caf.doctype.daily_output_record.daily_output_record.DailyOutputRecord.process_all")
    def test_background_completed_on_success(self, mock_process_all):
        """run_process_all_background sets Completed when process_all succeeds"""
        mock_process_all.return_value = {"success": True, "message": "OK"}
        self.dor.save()

        from caf.caf.doctype.daily_output_record.daily_output_record import run_process_all_background
        run_process_all_background(self.dor.name)
    ...
    @patch("caf.caf.doctype.daily_output_record.daily_output_record.DailyOutputRecord.process_all")
    def test_background_failed_with_comment(self, mock_process_all):
        """run_process_all_background sets Failed + adds comment when process_all fails"""
        mock_process_all.return_value = {
            "success": False, "failed_row": 1, "error": "Test error"
        }
        self.dor.save()
        ...
        error = frappe.db.get_value("Daily Output Record", self.dor.name, "processing_error")
