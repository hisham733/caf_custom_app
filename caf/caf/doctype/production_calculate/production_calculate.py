...
class ProductionCalculate(Document):
    ...
    def calculate_items_from_size(self):
        """
        Fetch BOM items for the recipe and return as list of dicts for JS.
        Does NOT save anything; purely in-memory.
        """
        if not self.recipe_name1:
                # Recipe empty → return empty table
                self.items = []
                return []

        # Fetch BOM items: list of tuples (item_code, qty, uom)
        bom_items = get_bom_items_for_item_as_list(self.recipe_name1)

        # Clear table in memory
        self.items = []

        # Append items to the in-memory table
        for code, qty, uom in bom_items:
                self.append("items", {
                "item_code": code,
                "qty": qty,
                "uom": uom
        ...
        # Return table as list of dicts for JS
        return [{"item_code": i.item_code, "qty": i.qty, "uom": i.uom} for i in self.items]

@frappe.whitelist()
def calculate_items_from_size_js(docname, recipe_name1=None):
    """
    Whitelisted method for JS call.
    Pass the recipe explicitly to avoid stale value.
    """
    doc = frappe.get_doc("Production Calculate", docname)

    # Use the passed recipe if provided
    if recipe_name1:
        doc.recipe_name1 = recipe_name1

    return doc.calculate_items_from_size()
...
@frappe.whitelist()
def get_bom_items_for_item_as_list(item_code):
...
def get_pack_boms_and_weights_for_client(recipe_bom_name):
    ...
    # Get the main Production Calculate document
    recipe_bom = frappe.get_doc("BOM", recipe_bom_name)
    recipe_qty = recipe_bom.quantity
    recipe_item_code = recipe_bom.item

    # Find related BOMs that include the same item as this Production Calculate
    ...
    for bom in pack_boms:
        ...
        if not weight_kg:
            frappe.throw(
                f"Missing weight_per_unit {weight_kg} for item {main_item_code}"
            )
...
def calculate_qty_from_size_api(item, size):
    doc = frappe.new_doc("Production Calculate")
...
@frappe.whitelist()
def get_recipe_items(custom_recipe):
    bom_items = get_bom_items_for_item_as_list(custom_recipe)
    return [{"item_code": code, "qty": qty, "uom": uom} for code, qty, uom in bom_items]